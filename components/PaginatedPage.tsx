...
const PaginatedPage = ({title, results}: PageProps) => {
        ...
        <div className="bg-slate-950 p-6 py-12">
            <div className="container max-w-6xl mx-auto py-4">
                <div className='title-wrap flex justify-between'>
                    <h3 className="self-start text-3xl font-bold pb-8 capitalize">{title || ''}</h3>
                ...
                <div className="card-con grid grid-cols-5 gap-7">
                    {results.data && results.data.map((data, idx) => (
                        <div key={idx}  className="anime-cards">
                            ...
                            <div className="ratings flex items-baseline pt-2">
                                <div className="score mr-2 font-bold text-2xl">{data?.score?.toFixed(2) || 'N/A'}</div>
