...
export const getStaticProps = async(context: PathParams) => {
    ...
    if(response?.pagination?.last_visible_page < slug[1]){
        return {
            notFound: true
        }
    }
...
const Search = ({results}: PageProps) => {
    const paginationOpts: PaginationPropsWithOpts = {
        totalPages: results.pagination?.last_visible_page,
        baseLink: `/search/${slug(results.slug[0])}`,
        name: slug(results.slug[0]),
        id: '',
        pageId: results.slug[1] || '1',
        responsiveNav: {
            desktop: 10,
            smallDesktop: 10,
            tablet: 5,
        ...
        <div className="bg-slate-950 p-6 py-12">
            <div className="container max-w-6xl mx-auto py-4">
                <div className='title-wrap flex justify-between'>
                    <h3 className="self-start text-3xl font-bold pb-8 capitalize">Results: {results.slug[0]}</h3>
